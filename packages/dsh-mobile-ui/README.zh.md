...
被裁模型名走真省略号 / 图标文字箭头垂直居中对齐」。它守的是第三条硬纪律——
...
- **操作行 = 两行布局**（2026-09-11 按用户选择实施）：
  ...
  改两行后模型名独占一行，同一视口可达 218px（38/40 字符），400px 以上**完整显示**；
  代价是行高 52 → 90px（多占约 38px 垂直空间）。
  权限按钮恒显盾牌图标 + 文字（`@container (min-width:170px)` 判据，第一行独占后
  320px 也放得下「完全权限」）；上下文 36px；被裁的模型名一律走**真省略号**。
