...
- **Composer action row**: never force a single line by "no-wrap + shrink" — upstream group buttons carry a non-compressible `min-width: 44px`, so under `flex-wrap: nowrap` a narrow row cannot resolve by shrinking and simply **overlaps** (the 2026-09-11 bug: at 390px the model trigger covered the context ring by 4.3px and the access-mode button covered the model trigger by 5.7px). The row keeps upstream `flex-wrap: wrap` as a fallback and lets the trailing group absorb the slack, leaving the model trigger as the only shrinkable item (ellipsis-truncated); access mode collapses to a 44px icon and the context meter to 36px. Below 360px the model trigger collapses to an icon, matching upstream's `@container` intent
- **Regression probe**: `python3 tests/mobile-layout-probe.py` drives a real headless Chromium (upstream CSS + real DOM nesting + the actual plugin bundle) across viewports, asserting two families of invariants: for the QA card — "option list scrolls / last option reachable after a real touch swipe / submit always visible / the fixed top-right menu button does not overlap the card"; for the composer row — "no two controls overlap / nothing spills past the card's right edge / every control is hit-testable". Exit code 1 on regression. Rules it guards: never override the card body's `overflow` (that is exactly what broke scrolling before); hide the floating menu button whenever the question card is open (it sits at `top:48px; right:12px`, right on top of the card title); never fake a single row with `nowrap` + shrink (use `wrap` plus one shrinkable item); and beware that `[class*=trigger]` also matches upstream's `_7KE1Ra_triggerLabel/Icon/Effort` — narrow it to `button[class*=trigger]`
